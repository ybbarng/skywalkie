 ...
 * 아이폰 앱이 만료되기까지 남은 날.
 *
 * 유료 개발자 계정이 없어서 **7일 뒤에는 앱이 안 열린다.**
 *
 * ## 화면에 띄우지 않기로 했다
 *
 * 아래 두 함수를 부르는 곳이 없는 것은 실수가 아니다.
 *
 * 7일을 넘겨 비행할 일이 없고, 대화 기록은 **안드로이드에서 뽑는다.**
 * 안드로이드에는 만료가 없으니 아이폰이 안 열려도 잃는 것이 없다.
 * 그래서 비행 중에 "곧 앱이 안 열려요" 를 띄워 겁줄 이유가 없다.
 *
 * 함수와 테스트는 남겨둔다. 사정이 바뀌어 아이폰에서도 뽑아야 하면
 * 그때 화면에 붙이면 된다.
 */
