 ...
 */
export function ConnectionBar({ state, pendingCount = 0, onPress }: ConnectionBarProps) {
  const theme = useTheme()
  const look = describe(state, pendingCount)
...
function describe(state: ConnectionState | null, pendingCount: number): BarLook {
  if (state === null) {
    return { color: 'textMuted', icon: 'refresh', text: '준비 중...', showSpinner: true }
  }

  const waiting = pendingCount > 0 ? ` · 보낼 것 ${pendingCount}개` : ''
    ...
    case 'connected':
      return state.link === 'ble'
        ? {
            color: 'warning',
            icon: 'bluetooth',
            text: '블루투스로 연결됨 · 글자만 가능',
            showSpinner: false,
          }
        : { color: 'success', icon: 'wifi', text: '연결됨', showSpinner: false }
