...
화면도 결국 저장소와 연결이 필요하다. 그런데 구현을 직접 가져오면 층 구분이 무너진다. 그래서 **`composition`을 한 번 거친다.**

```
presentation ──▶ composition ──▶ infrastructure
```

`composition/services.ts`가 화면이 쓸 것들을 모아 내보내고, 화면은 거기서 받는다. 이러면 "지금 무엇이 끼워져 있는지"를 아는 곳이 한 군데로 남는다.

대신 **`composition`에는 조립만 있고 판단이 없어야 한다.** 무엇을 할지 정하는 코드가 여기 들어오기 시작하면 이 구조가 의미를 잃는다. 실제로 이 규칙은 T12에서 화면을 만들다가 검사에 걸려 정해졌다.
